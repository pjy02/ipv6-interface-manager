...
1. 选择目标网络接口（如 eth0）
2. 输入IPv6网段前缀（如 `2602:f7c4:1:4f58`）
3. 设置子网掩码长度（默认64）
4. 指定地址范围（起始和结束后缀）
5. 确认配置并执行批量添加

**示例配置：**
- IPv6前缀: `2602:f7c4:1:4f58`
- 地址范围: 1-10
- 生成地址: `2602:f7c4:1:4f58::1/64` 到 `2602:f7c4:1:4f58::10/64`
...
假设您的服务器分配了IPv6网段 `2602:F7C4:1:4f58::/64`，您想要添加地址 `::1` 到 `::100`：

1. 运行脚本: `sudo ./ipv6_batch_config.sh`
2. 选择 "2. 批量添加IPv6地址"
3. 选择网络接口 (通常是 eth0)
4. 输入IPv6前缀: `2602:f7c4:1:4f58`
5. 子网掩码: `64` (默认)
6. 起始地址后缀: `1`
7. 结束地址后缀: `100`
8. 确认并执行
...
**传统方式（需要逐个执行）：**
```bash
ip -6 addr add 2602:f7c4:1:4f58::1/64 dev eth0
ip -6 addr add 2602:f7c4:1:4f58::2/64 dev eth0
ip -6 addr add 2602:f7c4:1:4f58::3/64 dev eth0
# ... 继续手动添加
```
