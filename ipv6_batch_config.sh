...
persist_addresses() {
    ...
    log INFO "持久化目标: $file (写入 iface 块内保证重启后生效)"
...
import sys, re
path, iface, ts, gateway, *addrs = sys.argv[1:]
...
auto_line = f"auto {iface}"
iface_re = re.compile(r'^(\s*)iface\s+%s\s+inet6\s+\S+' % re.escape(iface))
found_iface_idx = None
indent = ''
...
if found_iface_idx is None:
    # 如果不存在 IPv6 块，追加一个 static 块
    indent = ''
    lines.append(auto_line)
    lines.append(f"iface {iface} inet6 static")
    lines.append(f"{indent}    accept_ra 0")
    found_iface_idx = len(lines) - 2

# 找到当前 iface 块的结束位置
insert_idx = len(lines)
for j in range(found_iface_idx + 1, len(lines)):
    if re.match(r'^(auto|iface)\b', lines[j]):
        insert_idx = j
        break
...
block = [
    inner_indent + f"# BEGIN ipv6-batch-{iface}",
    inner_indent + f"# Generated by ipv6_batch_config.sh at {ts}",
]

for addr in addrs:
    block.append(inner_indent + f"post-up /sbin/ip -6 addr add {addr} dev {iface}")
    block.append(inner_indent + f"pre-down /sbin/ip -6 addr del {addr} dev {iface}")

if gateway:
    block.append(inner_indent + f"# persisted-gateway {gateway}")
    block.append(inner_indent + f"post-up /sbin/ip -6 route replace default via {gateway} dev {iface}")
    block.append(inner_indent + f"pre-down /sbin/ip -6 route del default via {gateway} dev {iface} 2>/dev/null")
else:
    block.append(inner_indent + "# persisted-gateway auto-detect")
    block.append(inner_indent + "post-up /bin/sh -c 'gw=$(ip -6 neigh show dev \"" + iface + "\" | awk \"/router/ {print $1; exit}\"); [ -n \"$gw\" ] && /sbin/ip -6 route replace default via \"$gw\" dev \"" + iface + "\"'"")

block.append(inner_indent + f"# END ipv6-batch-{iface}")

# 保证 auto 行存在
if auto_line not in lines:
    lines.insert(0, auto_line)
    insert_idx += 1
