...
ensure_interfaces_include() {
    if [[ ! -d "$PERSIST_DIR" ]]; then
        log ERROR "未找到 $PERSIST_DIR，无法写入持久化配置"; return 1
    fi
    if ! grep -Eq '^source /etc/network/interfaces\.d/\*' /etc/network/interfaces 2>/dev/null; then
        log WARN "主配置 /etc/network/interfaces 未包含 interfaces.d 目录，持久化配置可能无效"
        return 1
    fi
    return 0
}
