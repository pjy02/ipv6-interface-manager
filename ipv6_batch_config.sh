...
out = []
skip = False
replaced_iface = False

def iface_start(line):
    return re.match(r'^iface\s+\S+\s+inet6\b', line)
...
for idx, line in enumerate(lines):
    ...
    if skip:
        # 遇到下一个接口/auto 开始结束跳过
        if re.match(r'^(auto|iface)\b', line):
            skip = False
            out.append(line)
        continue
    out.append(line)

if not replaced_iface:
