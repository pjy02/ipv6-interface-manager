...
persist_addresses() {
    ...
    log INFO "持久化目标: $file (写入主配置以确保重启后生效)"
    local ts base_gw
    ts=$(date -Iseconds)
    base_gw="$gateway"
...
# 查找第一个 iface <iface> 块
iface_re = re.compile(r'^(\s*)iface\s+%s\s+\S+' % re.escape(iface))
...
# 计算插入位置（当前 iface 块结束前）
insert_idx = None
if found_iface_idx is not None:
    ...
    # 若不存在 iface 块，创建一个基础块
    indent = '    '
    lines.append(auto_line)
    lines.append(f"iface {iface} inet6 manual")
    found_iface_idx = len(lines) - 1
    insert_idx = len(lines)
...
for addr in addrs:
    block.append(inner_indent + f"pre-up /sbin/ip -6 addr add {addr} dev {iface}")
    block.append(inner_indent + f"post-down /sbin/ip -6 addr del {addr} dev {iface}")
...
block.append(inner_indent + detector)
block.append(inner_indent + remover)
block.append(inner_indent + f"# END ipv6-batch-{iface}")

# 若不存在 auto 行，则在文件顶部插入
