...
# ---------- 持久化 ----------
persist_file_for_iface() { echo "$PERSIST_DIR/ipv6-batch-$1.cfg"; }

interfaces_include_dir() {
    grep -Eq '^source /etc/network/interfaces\.d/\*' /etc/network/interfaces 2>/dev/null
}

persist_addresses() {
    local iface="$1"; shift; local -n arr=$1
    local gateway="${2:-}"
    local file use_main=0
    if [[ ! -d "$PERSIST_DIR" ]] || ! interfaces_include_dir; then
        log WARN "主配置缺少 interfaces.d 引用或目录不存在，改为写入 /etc/network/interfaces"
...
def is_iface_line(line):
    return re.match(r'^(auto|iface)\b', line)

def is_target_inet6(line):
    return re.match(r'^iface\s+%s\s+inet6\b' % re.escape(iface), line)
...
for line in it:
    ...
    # 移除已有 inet6 配置块
    if is_target_inet6(line):
        for line in it:
            if is_iface_line(line):
                out.append(line)
                break
        else:
            break
...
PYMAIN
        ...
        {
            echo "# Generated by ipv6_batch_config.sh at $ts"
